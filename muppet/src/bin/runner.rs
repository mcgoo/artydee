...
fn create_by_progid() {
    ...
    let rtd_server_progid = U16CString::from_str("Haka.PFY").unwrap();
...
fn create_by_clsid() {
    ...
    let cat = create_instance::<IRtdServer>(&CLSID_DOG_CLASS).expect("Failed to get a cat");
    use_rtd_server(cat);
